#!/usr/bin/perl
# https://github.com/Dimonous/nodeny49-tyme
# based on Tyme module by Dmytro Vasyk (https://github.com/Dimonous)
# Dmytro Vasyk
# dima@osokorky.net
...
$log_file='/usr/local/nodeny/mono-term.log';
...
&Ret(3, 'Payment already exist with txn_id: ' . $pp->{id}) if $pp;
...
$pp = &sql_select_line($dbh, "SELECT id as pay_id, FROM_UNIXTIME(UNIX_TIMESTAMP(), '%Y%m%d%H%i%s') as date FROM pays WHERE category='$category' AND reason='$txn_id'");
$txn_date = $pp->{date};
$pay_id = $pp->{pay_id};

&Ret(0, "Pay added to billing account:$mid txn_id:$pay_id txn_date:$txn_date sum:$sum" . ($sum_ok ? '' : " (WARNING: sum is 0)"));
